...
fn ensure_inherent_config(workspace_path: &str) -> Result<(), String> {
    ...
    // Ensure MCP section contains playwright, chrome-control, and autoui
    {
        ...
        if needs_introspect {
            ...
            // In dev: <repo>/apps/desktop/binaries/teamclaw-introspect-<triple>
            ...
            let introspect_bin = std::env::current_exe()
                ...
                .and_then(|dir| {
                    // Dev mode: exe is in .cargo-target/debug/, binaries are in apps/desktop/binaries/
...
///
/// MCP server commands that reference bundled desktop binaries may contain a
                    ...
                    if let Some(arr) = command.as_array_mut() {
                        for item in arr.iter_mut() {
                            if let Some(cmd_str) = item.as_str() {
                                ...
                                if !cmd_str.contains("apps/desktop/binaries/")
                                    && !cmd_str.contains("src-tauri/binaries/")
                                {
                                    continue;
                                }
