...
mod tests {
    ...
    // forward from a prior agent_runtimes row. Direct HTTP-mocked coverage
    // for the helper used to live here against `SupabaseBackend`; the
    // equivalent behaviour is now covered by `CloudApiBackend`'s own
    // mock-server tests in `crate::backend::cloud_api`.
