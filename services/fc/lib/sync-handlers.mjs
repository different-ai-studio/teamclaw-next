...
async function litellmFetch(path, method, body) {
  ...
  if (!res.ok) throw Object.assign(new Error(`LiteLLM ${path} → ${res.status}`), { status: res.status, data });
  return data;
}

/** Slugify a team name, e.g. "My Team" → "my-team" */
...
// ---------------------------------------------------------------------------
// §3.0  POST /sync/create-team
// ---------------------------------------------------------------------------

/**
 * @param {string} userId  - verified from JWT (no team yet)
 * @param {object} body    - { teamName, nodeId? }
 */
export async function handleSyncCreateTeam(userId, body) {
  const { teamName, nodeId } = body || {};
  if (!teamName || typeof teamName !== 'string') {
    return json(400, { error: 'teamName is required' });
  }

  // 1. LiteLLM first (spec §3.0: failure → no DB rows written)
  let litellmTeamId, litellmKey;
  try {
    const teamRes = await litellmFetch('/team/new', 'POST', {
      team_alias: slugify(teamName),
      max_budget: 1,
      budget_duration: '30d',
    });
    ...
    const keyRes = await litellmFetch('/key/generate', 'POST', {
      ...
      key_alias: `${slugify(teamName)}-default`,
      max_budget: 0.5,
      budget_duration: '30d',
    });
    litellmKey = keyRes.key;
  } catch (e) {
    console.error('[sync/create-team] LiteLLM error:', e.message);
    return json(502, { error: `LiteLLM provisioning failed: ${e.message}` });
  }

  const aiGatewayEndpoint = AI_GATEWAY_ENDPOINT();
  const supabase = createServiceRoleClient();

  // 2. DB transaction via supabase-js sequential ops (all service_role, autocommit)
  // Slug collision: try base slug, then slug-1, slug-2, …
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    if (teamErr.code !== '23505') {
      return json(500, { error: `Failed to create team: ${teamErr.message}` });
    }
  }
  if (!teamId) {
    return json(500, { error: 'Could not generate a unique team slug after 10 attempts' });
  }

  // Insert actor for this user in the new team
  const { data: actorRow, error: actorErr } = await supabase
    .from('actors')
    .insert({
      team_id: teamId,
      actor_type: 'member',
      display_name: teamName + ' Owner',
      user_id: userId,
      node_id: nodeId || null,
    })
    .select('id')
    .single();
  if (actorErr) {
    return json(500, { error: `Failed to create actor: ${actorErr.message}` });
  }
  const actorId = actorRow.id;

  // Insert members row (members.id = actors.id per spec §3.0)
  const { error: memberErr } = await supabase
    .from('members')
    .insert({ id: actorId, status: 'active' });
  if (memberErr) {
    return json(500, { error: `Failed to create member: ${memberErr.message}` });
  }

  // Insert team_members
  const { error: tmErr } = await supabase
    .from('team_members')
    .insert({ team_id: teamId, member_id: actorId, role: 'owner' });
  if (tmErr) {
    return json(500, { error: `Failed to create team_member: ${tmErr.message}` });
  }

  // Insert team_workspace_config (sync_mode='oss', oss_change_seq=0)
  const { error: twcErr } = await supabase
    .from('team_workspace_config')
    .insert({
      team_id: teamId,
      sync_mode: 'oss',
      oss_change_seq: 0,
      litellm_team_id: litellmTeamId,
      ai_gateway_endpoint: aiGatewayEndpoint,
    });
  if (twcErr) {
    return json(500, { error: `Failed to create team_workspace_config: ${twcErr.message}` });
  }

  return json(200, { teamId, teamSlug, aiGatewayEndpoint, litellmKey });
}
