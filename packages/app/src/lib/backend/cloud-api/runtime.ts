...
export function createRuntimeModule(client: CloudApiClient, delegate: RuntimeBackend): RuntimeBackend {
  return {
    ...
    async listSessionRuntimeModels(sessionId) {
      const out = await client.get<{ items: SessionRuntimeModelRow[] }>(`/v1/sessions/${encodeURIComponent(sessionId)}/runtime-models`);
      return out.items;
    },
    async listRuntimeTargetsForSession(sessionId, agentActorIds) {
      const params = new URLSearchParams();
      for (const id of agentActorIds) params.append("agentId", id);
      const out = await client.get<{ items: RuntimeTargetRow[] }>(`/v1/sessions/${encodeURIComponent(sessionId)}/runtime-targets?${params}`);
      return out.items;
    },
