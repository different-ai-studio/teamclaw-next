...
export function IdeasView() {
  ...
  const [creators, setCreators] = React.useState<CreatorMap>(new Map())
  const [loading, setLoading] = React.useState(false)
  const [error, setError] = React.useState(false)

  // Resolve team_id: prefer existing session row, else query actors WHERE user_id = current user.
  React.useEffect(() => {
    if (teamId) return
  ...
  // Fetch ideas + creators when team_id is known.
  React.useEffect(() => {
    if (!teamId) return
    let cancelled = false
    setLoading(true)
